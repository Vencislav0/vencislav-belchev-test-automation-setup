...
describe('eMAG Tests e2e', () => {
  ...
  it('Should correctly perform search on items and favorites page and icons should work as expected', async () => {
    ...
    let firstProductTitle = await dronesPage.getFirstProductTitle()
    let firstProductPrice = await dronesPage.getFirstProductPrice()
    let secondProductTitle = await dronesPage.getSecondProductTitle()
    let secondProductPrice = await dronesPage.getSecondProductPrice()
