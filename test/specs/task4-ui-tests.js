...
describe('Add/Remove Elements e2e', () => {
  it('should add and remove elements when clicking the buttons', async () => {
    ...
    let deleteButtonArray = await $$('//button[text()="Delete"]')

    assert.equal(deleteButtonArray.length, 10, 'Delete buttons should be 10 after clicking add element 10 times')

    for (let i = 10; i > 0; i--) {
      const lastDeleteButton = deleteButtonArray[deleteButtonArray.length - 1]
      await lastDeleteButton.click()

      deleteButtonArray = await $$('//button[text()="Delete"]')
      assert.equal(deleteButtonArray.length, i - 1, 'Delete button should be removed when clicked')
    }
  ...
  it('Confirming functionality of all alerts displayed', async () => {
    ...
    await browser.waitUntil(
      async () => {
        ...
        await browser.acceptAlert()
        return (await $('//p[@id="result"]').getText()) == 'You entered: test'
      },
      { timeout: 5000, interval: 500, timeoutMsg: 'Failed to input text in alert field' },
    )
...
describe('Hovers e2e', () => {
  it('Confirming functionality of Hovers', async () => {
    ...
    assert.include(await names[0].getText(), 'user1', 'First name of the hovered image should be correct')
    ...
    assert.include(await names[1].getText(), 'user2', 'Second name of the hovered image should be correct')
    ...
    assert.include(await names[2].getText(), 'user3', 'Third name of the hovered image should be correct')
    ...
    assert.equal(await $('h1').getText(), 'Not Found', 'the header text should be "Not Found"')
  })
})
